    ...
    s_handle_subject_asset_manipulation (fty_asset_server_t *cfg, zmsg_t **zmessage_p)
{
    ...
    try
    {   
        if (0 == cfg->limitations->global_configurability) {
            throw std::runtime_error("Licensing limitation hit - asset manipulation is prohibited.");
        }

        //get Asset Object
        Asset asset = ftyProtoToAsset(fmsg, read_only);

        //Create
        if(streq (operation, "create") || streq (operation, "create-force"))
        {
            //get options
            bool tryActivate = streq (operation, "create-force");

            //Create the asset
            //Asset createdAsset = createAsset(asset, tryActivate);

            //Notify the clients
            //notifyCreateAsset(createdAsset);
        }
        else if(streq (operation, "update"))
        {
            //get asset
            //Asset assetBeforeUpdate = getAsset(asset.getInternalName());

            //Update the asset
            //Asset assetAfterUpdate = updateAsset(asset, false);

            //Notify the clients
            //notifyUpdateAsset(assetBeforeUpdate, assetAfterUpdate);
        }
